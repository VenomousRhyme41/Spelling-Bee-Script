...
function b.OnInvoke(response) if response == "Copy Link" then if setclipboard then setclipboard("https://dsc.gg/morsteam") end end end
game:GetService("StarterGui"):SetCore("SendNotification", {Title = "Mors Team", Text = "Join my discord server", Duration = 5, Callback = b, Button1 = "Copy Link"})
...
-- UI properties
Frame.Position = UDim2.new(1, -315, 1, -250)  -- Bottom right of the screen
Frame.Size = UDim2.new(0, 300, 0, 250)  -- Adjusted height for all elements
...
TitleLabel.Text = "Spelling Bee Cheats"
TitleLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
TitleLabel.TextScaled = true

-- Combined subtitle and link on one line
local SubTitleLinkLabel = Instance.new("TextLabel", Frame)
SubTitleLinkLabel.Position = UDim2.new(0, 0, 0, 50)
SubTitleLinkLabel.Size = UDim2.new(1, 0, 0, 20)  -- Adjusted size for closer spacing
SubTitleLinkLabel.BackgroundTransparency = 1
SubTitleLinkLabel.Text = "Mors Team - dsc.gg/morsteam"
SubTitleLinkLabel.TextColor3 = Color3.fromRGB(255, 0, 0)
SubTitleLinkLabel.TextScaled = true

local WordLabel = Instance.new("TextLabel", Frame)
WordLabel.Position = UDim2.new(0, 0, 0, 70)  -- Closer spacing with the previous label
WordLabel.Size = UDim2.new(1, 0, 0, 50)
WordLabel.BackgroundTransparency = 1
WordLabel.Text = "Current Word: None"
WordLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
WordLabel.TextScaled = true

-- Create a highlight effect for the current word
local function highlightWord()
    WordLabel.TextColor3 = Color3.fromRGB(0, 255, 0) -- Change text color to green for highlight
    task.wait(0.5) -- Duration of highlight
    WordLabel.TextColor3 = Color3.fromRGB(255, 255, 255) -- Reset text color to white
end

local CopyButton = Instance.new("TextButton", Frame)
CopyButton.Position = UDim2.new(0.1, 0, 0, 130)
CopyButton.Size = UDim2.new(0.8, 0, 0, 40)
CopyButton.BackgroundColor3 = Color3.fromRGB(100, 100, 100)
CopyButton.Text = "Copy Word (F3)"

local AutoTypeToggle = Instance.new("TextButton", Frame)
AutoTypeToggle.Position = UDim2.new(0.1, 0, 0, 180)
AutoTypeToggle.Size = UDim2.new(0.8, 0, 0, 40)
AutoTypeToggle.BackgroundColor3 = Color3.fromRGB(100, 100, 100)
AutoTypeToggle.Text = "Enable Auto Type"

-- Variables for auto-typing
local typingdelay = 0.05  -- Set to a faster typing speed (50 milliseconds)
local autotypeenabled = false
local lastsound = nil
local lastword = "None"
local istypingmaybe = false
local typingword = false
local pretypewait = 0.5
...
local wordlist = {
	...
	["rbxassetid://137371985991938"] = "cognit",
	["rbxassetid://137371985991938"] = "sniffle",
...
local function autotypeword(word)
    ...
    for letter in word:gmatch(".") do
        ...
        presskey(letter)
        task.wait(typingdelay)  -- Uses the updated fast typing delay
    end
    if typingword then
        task.wait(typingdelay)  -- Wait before pressing enter
        presskey("enter")
    end
    typingword = false
    istypingmaybe = false
...
local function updatecurrentword(word)
    ...
    lastword = word
    WordLabel.Text = "Current Word: " .. word
    highlightWord()  -- Highlight the word when updated
...
CopyButton.MouseButton1Click:Connect(function()
    ...
    print("Copied: " .. lastword) -- Use print for console feedback
end)

AutoTypeToggle.MouseButton1Click:Connect(function()
    autotypeenabled = not autotypeenabled
    AutoTypeToggle.Text = autotypeenabled and "Disable Auto Type" or "Enable Auto Type"
end)

-- Toggle visibility with left Alt key and copy with F3 key
...
UserInputService.InputBegan:Connect(function(input, isProcessed)
    ...
    elseif input.KeyCode == Enum.KeyCode.F3 then
        setclipboard(lastword)
        print("Copied (via F3): " .. lastword) -- Use print for console feedback
    end
end)
